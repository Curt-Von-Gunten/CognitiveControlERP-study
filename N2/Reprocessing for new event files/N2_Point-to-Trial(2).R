...
for (k in sublist) {
  ...
  temp2 = read.delim(paste(path, "Created by R_ArtRej/N2/", k, "_N2_PBP_(220-320).txt", sep=""), header=T) 
  ...
  write.table(avgTemp, paste(path,"Created by R_ArtRej/N2/", k, "_TBT_N2.txt", sep=""), sep = "\t", row.names = F)
  
}

write.table(QuantDat, paste(path,"Created by R_ArtRej/N2/AllSubs_TBT_N2_10subs.txt", sep=""), sep = "\t", row.names = F)
