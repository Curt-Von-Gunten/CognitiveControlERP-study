...
sublist <- c("002", "004", "008", "016", "017", "019", "030", "059", "070", "074")
...
for (k in sublist) { # If you're not using all subjects, you can specify a list of which subjects for which
  ...
  ev2 = read.delim(paste(path, "Event_Stimulus_Files/Modified_Event_Files/", k, "flk_rev_manuallymodifiedHVE.ev2", sep=""), header=FALSE)
  ...
  write.table(temp.select, paste(path, "Created by R_ArtRej/N2/", k, "_N2_PBP_(220-320).txt", sep=""),
              sep="\t", row.names = F)
  
  write.table(numAccTemp, paste(path,"Created by R_ArtRej/N2/", k, "_N2_NumberOfAcceptedTrials.txt", sep=""), sep = "\t", row.names = F)
}

write.table(NumAcceptedTrials, paste(path,"Created by R_ArtRej/N2/N2_NumberOfAcceptedTrials_10subs.txt", sep=""), sep = "\t", row.names = F)
