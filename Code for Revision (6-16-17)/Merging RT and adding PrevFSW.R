...
N1_N2_FSW_dat = read.delim(paste(path, "Revision Data Sets/","N1_N2_FSW.txt", sep = ""))
...
RT_N1_N2_FSW_dat_allERP <- merge(RTdatshort, N1_N2_FSW_dat, by = c("Subject", "Trial")) 

#Remove "notfirst" and precCondTrigger.
Alldat <- RT_N1_N2_FSW_dat_allERP[,c(-10)]

colnames(Alldat) <- c("Subject", "Trial", "RT", "Electrode", "N1Curr", "AccCurr", "CondCurr", "TriggerCurr",
                      "N1Prev", "TriggerPrev", "AccPrev", "Cell", "CompleteRep", "AnyRep", "TargetRep",
                      "FlankerRep", "CondPrev", "N2Curr", "SlowWaveCurr")

#For comparison purposes
#Sub2 <- RT_N1_N2_FSW_dat_allERP[RT_N1_N2_FSW_dat_allERP$Subject == 2,]
...
#Sub137 <- RT_N1_N2_FSW_dat_allERP[RT_N1_N2_FSW_dat_allERP$Subject == 137,]
#####################################################################
...
write.table(Alldat, paste(path, "Revision Data Sets/RT_N1_N2_FSW_FSWPrev.txt", sep=""), sep = "\t", row.names = F)

#Adding FSW columns after a mess up. Don't want to rerun the block above.
FSWPrev <- Alldat[,c()]

aaaa <- Alldat[Alldat$Subject == 137,]
aaaa
